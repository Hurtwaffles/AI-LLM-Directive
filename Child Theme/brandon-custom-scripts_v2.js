...
// === P5.JS Merged Logic (Wave + Haze + Dot Grid + Overlay) ===
(function() {
  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  const BRANDON_CONFIG = {
    dot: { gap: 15, size: 1.5 },
    wave: { dotColor: [42, 42, 46], thickness: 145, speed: 247 },
    haze: {
  ...
  function buildDotGrid(p, { dotGap = 15, dotSize = 1.5 }) {
    ...
    const offsetX = (p.width - cols * dotGap) / 2;
    const offsetY = (p.height - rows * dotGap) / 2;
    return Array.from({ length: cols * rows }, (_, i) => {
      const x = offsetX + (i % cols) * dotGap;
      const y = offsetY + Math.floor(i / cols) * dotGap;
      return { x, y, size: dotSize };
    });
  }

  function drawGridOverlay(p, dotGap = 15) {
    const cols = Math.floor(p.width / dotGap);
    const rows = Math.floor(p.height / dotGap);
    p.push();
  ...
  function createHazeSketch(container, theme = 'dark') {
    return function(p) {
      const { bg, dot } = BRANDON_CONFIG.haze[theme];
      const BLOB_SCALE = 0.005, THRESHOLD = 0.64, FADE_RANGE = 0.17, SPEED = 0.0002;
      let dots = [];
      ...
      p.setup = () => {
        ...
        prefersReducedMotion ? p.noLoop() : p.loop();
      };

      p.draw = () => {
        p.background(bg);
        const t = p.millis() * SPEED;
        for (let { x, y, size } of dots) {
          const n = p.noise(x * BLOB_SCALE, y * BLOB_SCALE, t);
          const alpha = Math.max(0, Math.min(1, (n - (THRESHOLD - FADE_RANGE)) / (2 * FADE_RANGE)));
          if (alpha > 0.02) {
            p.fill(...dot, alpha * 255);
  ...
  function createWaveSketch(container) {
    return function(p) {
      let dots = [];
      ...
      p.draw = () => {
        p.background(0);
        const t = p.millis() * 0.001, cx = p.width / 2, cy = p.height / 2;
        for (let { x, y, size } of dots) {
          const d = p.dist(x, y, cx, cy);
          const alpha = 0.5 * (1 + Math.sin((d / thickness) - (t * speed * 0.001)));
          if (alpha > 0.01) {
            p.fill(...dotColor, alpha * 255);
            p.ellipse(x, y, size * 2, size * 2);
          }
  ...
  function injectBackgrounds() {
    document.querySelectorAll("smp-section.brandon-p5-bg").forEach(section => {
      let type = "dark";
      if (section.classList.contains("haze-light")) type = "light";
      else if (section.classList.contains("ripple-wave")) type = "wave";

      const container = document.createElement("div");
      container.className = "brandon-bg-canvas";
      section.prepend(container);

      const sketch = (type === "wave")
        ? createWaveSketch(container)
        : createHazeSketch(container, type);
